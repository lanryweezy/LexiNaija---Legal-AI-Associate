    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10 animate-in fade-in duration-1000">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
            ...
            <h2 className="text-5xl font-serif font-black text-legal-900 dark:text-white italic tracking-tighter leading-tight">Good Morning,<br/>Counsel.</h2>
            <p className="text-slate-400 dark:text-slate-500 font-medium">Your legal workspace is synchronized and <span className="text-legal-900 dark:text-legal-gold font-bold">Optimal</span>.</p>
        </div>
        <div className="flex flex-col items-end shrink-0">
            <div className="bg-legal-900 dark:bg-slate-900 text-white rounded-[32px] p-1.5 flex items-center shadow-2xl ring-4 ring-legal-900/5 dark:ring-white/5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        {limitationCases.length > 0 && (
            <div className="md:col-span-3 space-y-4">
                {criticalCases.length > 0 && (
                    <div className="bg-rose-50 dark:bg-rose-950/20 p-6 rounded-[24px] shadow-sm border-2 border-rose-200 dark:border-rose-900/50 animate-pulse">
                        <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-4 flex-1">
                                <div className="w-14 h-14 bg-rose-100 dark:bg-rose-900/50 rounded-2xl flex items-center justify-center text-rose-600 dark:text-rose-400 shrink-0">
                                    <AlertTriangle size={28} />
                                </div>
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2">
                                        <h3 className="text-rose-900 dark:text-rose-100 font-black text-xl font-serif italic tracking-tight">CRITICAL: Statute of Limitations Alert</h3>
                                    ...
                                    <p className="text-rose-700 dark:text-rose-300 text-sm font-medium mb-3">
                                    ...
                                    <div className="space-y-2">
                                        {criticalCases.slice(0, 3).map(c => (
                                            <div key={c.id} className="bg-white dark:bg-slate-900 rounded-xl p-3 flex items-center justify-between border border-rose-100 dark:border-rose-900/30">
                                                <div>
                                                    <p className="font-bold text-rose-900 dark:text-rose-100 text-sm">{c.title}</p>
                                                    <p className="text-xs text-rose-600 dark:text-rose-400 font-black">
                                                        {c.urgency.message}
                                                    </p>
                                                </div>
                                                <button 
                                                    onClick={() => { setActiveCaseId(c.id); handleNavigate(AppView.CASES); }}
                ...
                {warningCases.length > 0 && criticalCases.length === 0 && (
                    <div className="bg-amber-50 dark:bg-amber-950/20 p-6 rounded-[24px] shadow-sm border border-amber-200 dark:border-amber-900/50">
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4 flex-1">
                                <div className="w-12 h-12 bg-amber-100 dark:bg-amber-900/50 rounded-xl flex items-center justify-center text-amber-600 dark:text-amber-400 shrink-0">
                                    <AlertCircle size={24} />
                                </div>
                                <div>
                                    <h3 className="text-amber-900 dark:text-amber-100 font-black text-lg font-serif italic tracking-tight">Limitation Warning</h3>
                            ...
                            <button onClick={() => handleNavigate(AppView.CASES)} className="bg-amber-600 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-700 transition-colors">
                                Review Docket
                            </button>
                        </div>
                    </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-gray-500 dark:text-slate-400 font-medium text-sm">Active Matters</h3>
            <AlertCircle className="text-legal-gold w-5 h-5" />
          </div>
          <p className="text-3xl font-bold text-legal-900 dark:text-white">{activeCases}</p>
          <p className="text-xs text-green-600 dark:text-green-400 mt-1">Files currently open</p>
        </div>
        
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-gray-500 dark:text-slate-400 font-medium text-sm">Research Queries</h3>
            <Clock className="text-legal-gold w-5 h-5" />
          </div>
          <p className="text-3xl font-bold text-legal-900 dark:text-white">48</p>
          <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">This month (Simulated)</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-gray-500 dark:text-slate-400 font-medium text-sm">Documents Drafted</h3>
            <FileText className="text-legal-gold w-5 h-5" />
          </div>
          <p className="text-3xl font-bold text-legal-900 dark:text-white">{totalDocuments}</p>
          <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">Across all active cases</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-10">
          {activeCase ? (
            <div className="bg-legal-900 dark:bg-slate-900 rounded-[40px] p-10 text-white relative overflow-hidden shadow-2xl border dark:border-slate-800">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-[40px] p-20 border-2 border-dashed border-slate-200 dark:border-slate-800 text-center">
                <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6">
                    <Briefcase className="text-slate-300 dark:text-slate-600" size={32} />
                </div>
                <h4 className="text-xl font-serif font-black text-slate-400 dark:text-slate-600 italic">No Matter in Focus</h4>
                <p className="text-xs text-slate-400 dark:text-slate-600 mt-2 uppercase font-black tracking-widest">Select a case to initialize deep intelligence analysis</p>
            </div>
          )}

          {/* Legislative Briefings */}
          <div className="space-y-6">
            <h3 className="text-[10px] font-black text-legal-900 dark:text-legal-gold uppercase tracking-[0.3em] flex items-center gap-3">
              <span className="w-8 h-px bg-legal-gold"></span>
              Legislative Intelligence Briefings
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm group hover:shadow-xl transition-all">
                    <div className="flex items-center gap-3 mb-4">
                        <div className="w-8 h-8 bg-sky-50 dark:bg-sky-900/20 rounded-lg flex items-center justify-center text-sky-500">
                            <AlertCircle size={16} />
                        </div>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">CAMA 2020 Protocol</span>
                    </div>
                    <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-medium italic">Annual returns for incorporated trustees are due by June 30th. AI drafting is available in Corporate Assistant.</p>
                </div>
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm group hover:shadow-xl transition-all">
                    <div className="flex items-center gap-3 mb-4">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-medium italic">New electronic filing protocols for FHC Lagos are now operational. Verifying compatibility with LexiNaija archiving.</p>
                </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <CounselAgent onNavigate={handleNavigate} />
            ...
            <div className="p-8 bg-slate-50 dark:bg-slate-900/50 rounded-[32px] border border-slate-100 dark:border-slate-800">
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center text-xs font-bold text-legal-900 dark:text-white">
