    ...
    <div className="p-8 max-w-7xl mx-auto h-[calc(100vh-2rem)] flex flex-col animate-in fade-in duration-1000">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 flex-1 overflow-hidden">
        ...
        <div className="lg:col-span-8 bg-white/40 backdrop-blur-xl rounded-[56px] border border-white shadow-2xl flex flex-col overflow-hidden relative">
          ...
          {selectedClient && (
             <div className="absolute inset-0 bg-white z-20 flex flex-col animate-in fade-in slide-in-from-bottom-10 duration-500">
                <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                        <span className="text-[10px] font-black text-legal-gold uppercase tracking-widest">Active Terminal</span>
                        <h3 className="text-3xl font-serif font-black italic text-legal-900 tracking-tight">{clients.find(c => c.id === selectedClient)?.name}</h3>
                    </div>
                    <button onClick={() => setSelectedClient('')} className="p-3 text-slate-400 hover:text-legal-900 bg-white border border-slate-100 rounded-2xl transition-all"><X size={20}/></button>
                </div>
                <div className="flex-1 overflow-y-auto p-10 space-y-10">
                    <div>
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Active Matters Progress</h4>
