    ...
    <div className="h-screen bg-slate-50 dark:bg-slate-950 flex flex-col overflow-hidden relative">
      ...
      <div className="h-20 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between px-10 shrink-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-20">
        <div className="flex items-center gap-4">
            ...
            <div>
                <h1 className="text-xl font-serif font-black text-legal-900 dark:text-white italic tracking-tighter uppercase">Firm Architecture & Protocol</h1>
                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest text-left">Internal Infrastructure Management</p>
            </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-12 relative z-10 scrollbar-hide">
        <div className="max-w-4xl mx-auto space-y-12">
          <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[48px] border border-white dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col">
            <div className="p-8 border-b border-slate-50 dark:border-slate-800 bg-white/30 dark:bg-white/5">
                <h3 className="text-2xl font-serif font-black text-legal-900 dark:text-white italic tracking-tighter">Practice Identity</h3>
                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">This metadata is utilized for automated instrument drafting and billing serialization.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-8">
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 ml-6">Firm Corporate Identity</label>
                    <div className="relative group">
                        ...
                        <input 
                          ...
                          className="w-full pl-16 pr-6 py-5 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-[20px] font-bold text-legal-900 dark:text-white focus:ring-4 focus:ring-legal-gold/5 outline-none transition-all shadow-inner"
                        />
                    </div>
                  </div>

                  <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 ml-6">Solicitor / Signatory Command</label>
                    <div className="relative group">
                ...
                <div className="flex flex-col">
                  <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 ml-6">Physical Jurisdiction (Head Office)</label>
                  <textarea 
                    value={formData.address}
                    onChange={e => setFormData({...formData, address: e.target.value})}
                    className="flex-1 w-full bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-[32px] p-8 text-lg font-serif italic text-legal-900 dark:text-white focus:ring-4 focus:ring-legal-gold/5 outline-none transition-all shadow-inner resize-none"
                    rows={4}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 ml-6">Firm Insignia (Logo URL)</label>
                    <div className="relative group">
                ...
                <div className="flex items-center gap-6 bg-slate-50 dark:bg-slate-800/50 p-6 rounded-[24px] border border-slate-100 dark:border-slate-700">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Institutional Mode</p>
                        <div className="flex items-center gap-3 mt-1 cursor-pointer" onClick={() => setFormData({...formData, isEnterprise: !formData.isEnterprise})}>
                            <div className={`w-10 h-5 rounded-full transition-all relative ${formData.isEnterprise ? 'bg-legal-900 dark:bg-legal-gold' : 'bg-slate-200 dark:bg-slate-700'}`}>
                                <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${formData.isEnterprise ? 'left-6' : 'left-1'}`}></div>
                            </div>
                            <span className="text-xs font-black text-legal-900 dark:text-white uppercase">Enterprise Activation</span>
                        </div>
                    </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 ml-6">Digital Correspondence</label>
                    <div className="relative group">
                ...
                <div>
                    <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 ml-6">Encrypted Comms Line</label>
                    <div className="relative group">
              ...
              <div className="pt-8 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <button
                    ...
                    className="p-4 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-slate-400 hover:text-legal-900 dark:hover:text-white rounded-[20px] hover:shadow-xl transition-all active:scale-95"
          ...
          <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[48px] border border-white dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col">
            <div className="p-8 border-b border-slate-50 dark:border-slate-800 bg-white/30 dark:bg-white/5 flex items-center justify-between">
                <div>
                    <h3 className="text-2xl font-serif font-black text-legal-900 dark:text-white italic tracking-tighter">Localized Wisdom Bank</h3>
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Manage institutional knowledge folders to improve AI research and drafting accuracy.</p>
                </div>
            ...
            <div className="p-10 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {['Entertainment', 'G.M IBRU', 'LAW SCHOOL', 'CAC', 'General'].map(cat => {
                  ...
                  return (
                    <div key={cat} className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-[32px] border border-slate-100 dark:border-slate-700 flex flex-col gap-4 group hover:bg-white dark:hover:bg-slate-800 hover:shadow-xl transition-all duration-500">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="bg-legal-gold/20 dark:bg-legal-gold/10 text-legal-900 dark:text-legal-gold text-[10px] font-black px-3 py-1 rounded-full group-hover:bg-legal-gold dark:group-hover:text-legal-900 transition-colors">{items.length} Folders</span>
                      </div>
                      ...
                      <div className="mt-auto pt-4 border-t border-slate-200/50 dark:border-slate-700/50 flex items-center justify-between">
                        <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500">Institutional Access: ON</span>
              ...
              <div className="mt-8 p-8 bg-legal-900 dark:bg-slate-800 rounded-[40px] flex items-center justify-between shadow-2xl relative overflow-hidden group">
